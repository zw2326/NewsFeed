# -*- coding: utf-8 -*
# Tool to test keyword extraction.
...
# Keyword from short text: https://towardsdatascience.com/extracting-keywords-from-short-text-fce39157166b
# Topic modeling?: http://mallet.cs.umass.edu
# Terminology extractor?
# Scrabble score of each word in a sentence, and taking the top Nth scoring words from those results and assume them to be the keywords?:
# https://towardsdatascience.com/extracting-keywords-from-short-text-fce39157166b

def ExtractKeywords(text):
    impl = EK_NEE # impl returns [(keyword1, score1), ...]
    # impl = EK_NPE
    return impl(text)
...
def EK_NEE(text):
    text = re.sub('<[^<]+?>', '', Translater.Translate(html.unescape(text)))
    ...
    return set(map(lambda x: (x, 0), entity_names))
...
def extract_entity_names(t):
    ...
    if hasattr(t, 'label') and t.label:
        if t.label() == 'NE':
            ...
            for child in t:
                ret.extend(extract_entity_names(child))
    return ret
...
if __name__ == '__main__':
    ...
    resource = sys.argv[1]
    n = int(sys.argv[2]) if len(sys.argv) >= 3 else float('inf')
    rawNewsItems = feedparser.parse(resource)
    newsItems = []
    for rawNewsItem in rawNewsItems.entries:
        title = re.sub('<[^<]+?>', '', Translater.Translate(html.unescape(rawNewsItem.title)))
        summary = re.sub('<[^<]+?>', '', Translater.Translate(html.unescape(rawNewsItem.summary)))
        kwpt = ExtractKeywords(title)
        kwps = ExtractKeywords(summary)

        newsItems.append({'title': title, 'summary': summary, 'kwpt': kwpt, 'kwps': kwps})

    for i, newsItem in enumerate(newsItems):
        if float(i) >= n:
            break

        print('[{0}] {1}'.format(i, newsItem['title']))
        for kwp in newsItem['kwpt']:
            print(' {0}  {1}'.format(' ' * len(str(i)), kwp))
        print(' {0}  {1}'.format(' ' * len(str(i)), newsItem['summary']))
        for kwp in newsItem['kwps']:
            print(' {0}  {1}'.format(' ' * len(str(i)), kwp))
