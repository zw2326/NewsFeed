...
class NewsItem(object):
    def __init__(self, source, entry):
        ...
        # DEBUG
        # open('inspect.txt', 'a', encoding='utf-8').write('{0}\n{1}\n{2}\n==========\n{3}\n++++++++++\n{4}\n{5}\n==========\n\n'.format(self.source.description, self.title, self.summary, list(self.keywords), self.kwt, self.kws))
        # DEBUG
        ...
        if env['isCacheEnabled'] == False or source.filecache is None:
            ...
            if not os.path.isfile(cacheFile) or env['forceCacheRefresh'] == True:
                ...
                try:
                    ...
                    Msg("Update file cache for {0} failed: {1}".format(source.description, e))
                    continue
...
# Get news items from a single news source.
def DownloadNewsSource(source):
    newsItems = []
    if env['isCacheEnabled'] == False or source.filecache is None:
        Debug('Load from web for {0}'.format(source.description))
        rawNewsItems = feedparser.parse(source.link)
    ...
    for i, newsGroupName in enumerate(env['newsGroups'].keys()):
        ...
        for newsItem in newsItems[newsGroupName]:
            ...
            newsItemStrs.append(newsItemStrTemp.format(
                ...
                SOURCE=html.escape(newsItem.source.description),
...
if __name__ == '__main__':
    ...
    i = 0
    newsGroups = []
    for k in env['newsGroups'].keys():
        print(k)
        for v in env['newsGroups'][k]:
            print('{0} {1}'.format(i, v.description))
            i += 1
            newsGroups.append(v)
        print('')
    i = int(input('> '))
    DownloadNewsSource(newsGroups[i])
